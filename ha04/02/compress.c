...
static float dct_sum(const float* input_block, uint8_t p, uint8_t q, float cosine_values[8][8])
{
	float sum = 0;
...
static void perform_dct(const float* input_block, float* output_block, float cosine_values[8][8])
{
	for (uint8_t p = 0; p < 8; p++)
		for (uint8_t q = 0; q < 8; q++)
			output_block[(8 * p) + q] = alpha(p) * alpha(q) * dct_sum(input_block, p, q, cosine_values);
}
